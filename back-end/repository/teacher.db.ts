...
const getAllTeachers = async (): Promise<Teacher[]> => {
    try {
        ...
        */
        const prismaTeacher = await database.teacher.findMany({
            include: {
                user: true,
            }
        });
        return prismaTeacher.map(teacher => Teacher.from(teacher));
    } catch (error) {
...
const updateLearningPath = async (teacherId: number, learningPath: string): Promise<Teacher> => {
    try {
        ...
        */
        const prismaTeacher = await database.teacher.update({
            where: {
                id: teacherId,
            },
            data: {
        ...
        return Teacher.from(prismaTeacher);
    } catch (error) {
        throw new Error('Database error. See server log for details.');
    }
};
